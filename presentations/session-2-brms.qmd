...
* Heavy tail means it accommodates very large standard deviations easily.
* Not regularizing enough if data is sparse or structure is complex.

**Recommended Prior:**
* **Exponential(2)** (mean = 0.5s).
* Mode at 0 provides **shrinkage**: pulls group variation toward 0 unless supported by data.
* **Prevents singular fits** (standard deviation estimating to exactly 0 or boundary) and stabilizes sampling.
:::
:::
:::
...
## Modeling response times with the Gaussian distribution in `brms` {.dark-bg background-color="#C2002F"}
...
For most parameters except for the coefficients, the default priors work well.
...
* A **95% Credible Interval** represents the range in which the true parameter value lies with **95% probability** (conditioned on the observed data and priors).
* *Interpretation:* If the interval **does not contain 0**, we can conclude that 0 is an implausible value for the effect (analogous to rejecting the null, but with a direct probability statement).

### 2. Probability of Direction ($pd$ / Proportion on One Side of 0)
* We can calculate the proportion of the posterior distribution that lies on one side of 0 (e.g., $P(\beta > 0 | \text{data})$).
* Calculated directly from the MCMC draws: `mean(draws > 0)` or `mean(draws < 0)`.
* *Interpretation:* Extremely intuitive. If $98.5\%$ of the posterior draws are positive, we can state there is a **$98.5\%$ probability that the effect is positive** (e.g., that the flanker condition slows participants down).

### 3. Bayes Factors (BF)
* Quantifies the **relative evidence** that the data provides for the alternative hypothesis ($H_1$) vs. the null hypothesis ($H_0$).
* A $BF_{10} = 15$ indicates that the observed data are 15 times more likely under $H_1$ than under $H_0$ (representing strong evidence for the effect).
* *Implementation:* In `brms`, can be estimated via the **Savage-Dickey density ratio** using `hypothesis(model, "Condition = 0")` or by using bridge sampling via the `bayes_factor()` function.
:::
...
::: {.accent-box}
* **$\beta$ (Exponential Scale)** is estimated globally on the raw scale: 
`r if (file.exists(here("presentations", "fit_exgaussian_mu.qs2"))) {
  fit_exg_mu <- qs_read(here("presentations", "fit_exgaussian_mu.qs2"))
  paste0(round(summary(fit_exg_mu)$spec_pars["beta", "Estimate"], 3), "s (approx. ", round(summary(fit_exg_mu)$spec_pars["beta", "Estimate"] * 1000), " ms of exponential tail latency)")
} else {
...
::: {.accent-box}
The overall fit is very similar to when we assume a single value of $\beta$ for all observations, but now we can see if beta varies between any of the conditions.
:::
