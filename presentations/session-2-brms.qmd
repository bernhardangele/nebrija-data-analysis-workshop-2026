...
::: {.accent-box}
**Rule of Thumb:** When in doubt, use the default priors, **except** for fixed effect coefficients (where you should always specify a regularizing prior like `normal(0, 0.1)`).

*Note: The exponential prior for standard deviations must be scaled according to your dependent variable's measurement scale (seconds vs. milliseconds).*
:::
...
$$\beta_{\text{CON\_AGR\_vs\_DIS}} = \frac{\text{CON} + \text{AGR}}{2} - \text{DIS}$$
...
* **Understanding `Evid.Ratio` (Bayes Factor):**
  * The **`Evid.Ratio`** column shows the Savage-Dickey Bayes Factor. Here, it is **`NA`** because `brms` requires prior samples to be saved during fitting (`brm(..., sample_prior = "yes")`).
  * **Manual Calculation:** Because it is `NA`, we can manually calculate it by comparing the prior density at 0 ($N(0, 0.1)$ density is $3.989$) to the posterior density at 0 ($0$ since draws are clustered away from 0):
    $$BF_{10} = \frac{\text{prior density at 0}}{\text{posterior density at 0}} = \frac{3.989}{0} = \infty$$
  * A $BF_{10} = \infty$ represents **extreme evidence** for the alternative hypothesis ($H_1$).
